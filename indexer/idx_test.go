...
func init() {
	...
	sch.ReadBytes(exampleSchema)
	exampleIndex, err = New(sch)
	if err != nil {
		panic(err)
	}
}

var exampleSchema []byte = []byte(`
...
objectClass: ( 2.5.17.0
        ...
        X-ORIGIN 'RFC3672' )
dITContentRule: ( 0.9.2342.19200300.100.4.7
	NAME 'roomContent'
        AUX simpleSecurityObject
        MUST cn
        MAY ( description $ roomNumber )
        NOT seeAlso )
